...
class Utils {
     ...
     * @param bool $modeStrict Only match on the first criteria (country or language). When there is no match return null
     * @return string Found locale
     */
    public function getLocaleWithFallback($locale, $searchLanguageFirst = false, $availableLocales = null, $modeStrict = false) {
        ...
        if (strlen($locale) === 5) {
            $fallbackLocale = $this->_tryFallbackLocales($locale, $searchLanguageFirst, $availableLocales, $modeStrict);
        ...
        if (strlen($locale) === 2) {
            ...
            $fallbackLocale = $this->_tryFallbackLocales(strtolower($locale).'_', $searchLanguageFirst, $availableLocales, $modeStrict);
            if ($fallbackLocale !== null) return $fallbackLocale;
        }

        // If everything failed return the default or the first available one
        if ($modeStrict) return null;
    ...
    private function _tryFallbackLocales($locale, $searchLanguageFirst = false, $availableLocales, $modeStrict) {
        ...
        if ($searchLanguageFirst) {
            ...
            if ($fallbackLocale === null && $modeStrict === false) $fallbackLocale = $this->_findLocale($availableLocales, $country, 1);
        } else {
            $fallbackLocale = $this->_findLocale($availableLocales, $country, 1);
            if ($fallbackLocale === null && $modeStrict === false) $fallbackLocale = $this->_findLocale($availableLocales, $language, 0);
        }
